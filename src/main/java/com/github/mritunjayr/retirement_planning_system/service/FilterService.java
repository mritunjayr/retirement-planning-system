...
public class FilterService {
    ...
    public FilterResponse filterTransactions(FilterRequest request) {
        ...
        for (ExpenseRequest expense : request.getTransactions()) {
            ...
            if (inAPeriod) {
                valid.add(new FilteredTransaction(expense.getDate(), expense.getAmount(), ceiling, remanent, true));
            } else {
                Map<String, Object> invalidTx = new HashMap<>();
                invalidTx.put("date", expense.getDate());
                invalidTx.put("amount", expense.getAmount());
                invalidTx.put("ceiling", ceiling);
                invalidTx.put("remanent", remanent);
    ...
    private double applyQPeriods(String date, double remanent, List<QPeriod> qPeriods) {
        ...
        for (int i = 0; i < qPeriods.size(); i++) {
            ...
            if (DateUtil.isInRange(date, q.getStart(), q.getEnd())) {
                LocalDateTime currentStart = DateUtil.parse(q.getStart());
    ...
    private boolean isInAnyKPeriod(String date, List<Period> kPeriods) {
